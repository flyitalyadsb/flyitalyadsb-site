...
function aircraftNearSkillMd(): string {
  return [
    ...
    '',
    'List every aircraft the FlyItalyADSB network currently sees within a radius of an Italian',
    'airport or a lat/lon point, via the `aircraft_near` MCP tool — full live records, not just',
    'a count.',
    '',
    '## Requirements',
    '',
    `- Connect to the MCP server at \`${MCP_URL}\` (Streamable HTTP, stateless).`,
    '- Call `aircraft_near` with either `airport` (a slug, IATA, or ICAO code, e.g. "roma-fiumicino",',
    '  "FCO", "LIRF") or both `lat` and `lon`.',
